...
describe('Utilities : listSpacer', () => {
  describe('listSpacer', () => {
    ...
    test('uses "and" when specified', () => {
      const length = 2
      expect(listSpacer({ index: 0, length, useAmpersand: false })).toBe(
        ' and ',
      )
      expect(listSpacer({ index: 1, length })).toBe('')
    })
  ...
  describe('stringifyList', () => {
    ...
    test('stringifies 3 items', () => {
      const stringifiedItems = stringifyList({
    ...
    test('stringifies more than 3 items', () => {
      const stringifiedItems = stringifyList({
        arr: [items[0], items[1], items[2], items[3]],
      })
      expect(stringifiedItems).toBe('Apple, Orange, Pear & Mango')
    })

    test('uses "and" when specified', () => {
      const formattedItems = stringifyList({
        arr: [items[0], items[1]],
        useAmpersand: false,
      })
